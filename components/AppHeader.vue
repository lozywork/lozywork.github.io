<template>
  ...
  <!-- Left hanger: location badge with globe icon -->
  ...
  <!-- Hero section: parallax background image + name overlay -->
...
// Current interpolated values — updated each animation frame
let imageY = 0.05;
...
// Store raw scroll position — kept separate from the animation loop
// so the RAF can interpolate smoothly without being tied to scroll events
const onScroll = () => {
  scrollY = window.scrollY;
};

const animate = () => {
  // Slowly drift the image down as the user scrolls
  imageY = lerp(imageY, scrollY * 0.15, SMOOTHNESS);
...
// Centers both hangers vertically on either side of the viewport
.hanger {
  height: fit-content;
  position: absolute;
  top: 50%;
...
// Location text inside the left hanger pill
p {
  position: absolute;
  top: 50%;
  transform: translateY(-50%);
...
// Globe button inside the hanger pill
.digital-ball {
  z-index: 500;
  position: absolute;
  right: 1.3em;
...
// Hangers slide up and fade in on page load
@keyframes fadeInUp {
